...
*   **AI Models**:
    *   **Logic/Text**: `glm-4-flash` / `glm-4-plus` (剧本分析)
    *   **Vision**: `cogview-4` / `dramabackend` (文生图与角色设计)
    *   **Video**: `cogvideox-flash` / `vidu2` (视频生成)
*   **Storage**: IndexedDB (本地浏览器数据库，数据隐私安全，无后端依赖)

## 内置 AI 模型

本项目内置多厂商 AI 模型支持，开箱即用：

### 🤖 智谱 AI (BigModel)
* **文本模型**: GLM-4-Plus、GLM-4-Air、GLM-4-Flash（免费）
* **图像模型**: CogView-4、CogView-3-Flash（免费）
* **视频模型**: CogVideoX-Flash（免费）、Vidu2、ViduQ1

### 🏗️ 自建服务 (WLDrama)
* **图像服务**: Drama Backend — 文生图、图生图、角色立绘、分镜生成

### 🔧 本地部署 (NewAPI)
* **兼容接口**: 支持接入 OpenAI 兼容的任意本地模型

所有模型均通过统一的三级 API Key 管理（模型专属 → 厂商级 → 全局兜底），支持在 UI 中自由切换和扩展。

## ⚠️ 开源与“免费”说明（请务必阅读）

* **模型使用说明**：本开源项目默认工作流需要使用对应能力的模型组合，例如大语言模型（如 **GLM-4-Flash**）、图像模型（如 **CogView-4**）以及视频模型（如 **CogVideoX-Flash**）；如需对接其他渠道或模型，可自行修改与适配。
...
1.  **配置密钥**: 启动应用，在"模型配置 → 厂商配置"中设置各厂商的 API Key。
