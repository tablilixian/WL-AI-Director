...
import { getActiveImageModel } from '../../../../services/ai/apiCore';
...
export class CanvasModelService {
  private async getApiConfig() {
    const { getApiBase, checkApiKey, resolveRequestModel } =
      await import('../../../../services/ai/apiCore');
    const { getActiveImageModel, getActiveVideoModel } =
      await import('../../../../services/ai/apiCore');
  ...
  async generateImage(options: GenerateImageOptions): Promise<string> {
    ...
    logger.debug(
      LogCategory.CANVAS,
      `[I2I:${traceId}] Generating image with ${provider}, isI2I=${isImageToImage}`,
    );
  ...
  async generateVideoMkr(options: GenerateVideoMkrOptions): Promise<string> {
    ...
    try {
      const { callDramaBackendVideoMkrApi } =
        await import('../../../../services/adapters/imageAdapter');

      onProgress?.(30);

      const videoMkrImages = images.map((img, _i) => ({
        image: img.src,
  ...
  async generateVideoMkrGrid(options: GenerateVideoMkrGridOptions): Promise<string> {
    ...
    try {
      const { callDramaBackendVideoMkrGridApi } =
        await import('../../../../services/adapters/imageAdapter');
      ...
      logger.error(
        LogCategory.CANVAS,
        '[CanvasModelService] MKR Grid video generation failed',
        error,
      );
      throw error;
    }
  ...
  async improvePrompt(prompt: string): Promise<string> {
    try {
      ...
      logger.warn(
        LogCategory.CANVAS,
        '[CanvasModelService] Prompt improvement failed, using original',
        error,
      );
      return prompt;
    }
  ...
  async enhancePrompt(
    ...
    const modePrompts: Record<string, string> = {
      'image-to-image':
        'Optimize this image editing prompt. Focus on specific visual changes while maintaining the original composition. Be precise about what to modify.',
      'style-transfer':
        'Enhance this style transfer prompt. Include specific artistic style details, color palettes, brush techniques, or visual characteristics.',
      'background-replace':
        'Improve this background replacement prompt. Describe the new environment in detail including lighting, atmosphere, and perspective.',
      expand:
        'Enhance this image expansion prompt. Describe what should naturally extend beyond the original boundaries.',
      'text-to-image':
        'Optimize this image generation prompt. Add vivid visual details, composition, lighting, and artistic style.',
    };
    ...
    try {
      ...
      logger.warn(
        LogCategory.CANVAS,
        '[CanvasModelService] Prompt enhancement failed, using original',
        error,
      );
      return prompt;
    }
  ...
  async styleTransfer(
    ...
    const stylePrompts: Record<string, string> = {
      anime:
        'Convert this image to anime style, maintaining the composition and key features. High quality anime illustration.',
      'oil-painting':
        'Convert this image to oil painting style with rich textures and brush strokes. Classic oil painting aesthetic.',
      watercolor:
        'Convert this image to watercolor painting style with soft edges and translucent washes.',
      ...
      cyberpunk:
        'Convert this image to cyberpunk style with neon lights, futuristic elements, and high-tech atmosphere.',
      ...
      'pop-art':
        'Convert this image to pop art style inspired by Andy Warhol and Roy Lichtenstein.',
    };
  ...
  async directStyleTransfer(
    ...
    try {
      const { callDramaBackendStyleTransferApi } =
        await import('../../../../services/adapters/imageAdapter');
      const traceId = this.generateTraceId();
  ...
  async inpaint(
    ...
    try {
      const { callDramaBackendInpaintApi } =
        await import('../../../../services/adapters/imageAdapter');
      const traceId = this.generateTraceId();
  ...
  async generate360Hdri(
    ...
    try {
      const { callDramaBackend360HdriApi } =
        await import('../../../../services/adapters/imageAdapter');
