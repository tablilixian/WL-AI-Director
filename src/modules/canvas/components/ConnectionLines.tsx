...
export const ConnectionLines: React.FC<ConnectionLinesProps> = ({
  ...
  void (selectedEdgeId ? selectedEdgeId.split('::')[0] : null);
  void (selectedEdgeId ? selectedEdgeId.split('::')[1] : null);
      ...
      {promptLayers.map((promptLayer) => {
        ...
        return (
          <React.Fragment key={`prompt-connections-${promptLayer.id}`}>
            {linkedLayerIds.map((linkedId, idx) => {
              ...
              return (
                <g key={`prompt-link-${promptLayer.id}-${linkedId}`}>
                  ...
                  />
                  <circle cx={midX} cy={midY} r={8} fill={promptColor} opacity={0.8} />
                  <text
                    x={midX}
                    y={midY + 4}
                    fill="#ffffff"
            ...
            {outputLayerIds.map((outputId, _idx) => {
              ...
              return (
                <g
                  key={`prompt-output-${promptLayer.id}-${outputId}`}
