  ...
  return (
    <div className="py-12 text-center space-y-4">
      ...
      <div className="flex gap-2 justify-center">
        ...
        <button onClick={onReset} className="px-6 py-2 border border-gray-600 text-gray-300 text-sm rounded-lg hover:text-white flex items-center gap-1.5">
      ...
      >
        ...
        <div className="flex-shrink-0 bg-gray-900 border-b border-gray-700 px-5 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={handleClose} className="p-1 hover:bg-gray-700 rounded text-gray-400 hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
                  ...
                  {PHASE_ORDER.slice(0, 5).map((phase, i) => (
                    <div key={phase} className="flex items-center gap-2">
                      <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${
                        ...
                        i === currentPhaseIndex ? 'bg-amber-900 text-amber-300 border border-amber-500' :
                        'bg-gray-800 text-gray-500 border border-gray-600'
                      }`}>
                        {i + 1}
                      </div>
                      <span className={`text-[10px] ${i <= currentPhaseIndex ? 'text-gray-300' : 'text-gray-500'}`}>
                        {PHASE_LABELS[phase]}
                      </span>
                      {i < currentPhaseIndex && (
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-gray-900">
              ...
              <div className="flex items-center gap-1">
                {PHASE_ORDER.slice(0, 5).map((phase, i) => (
                  <React.Fragment key={phase}>
                    ...
                    {i < 4 && (
                      <div className={`flex-1 h-0.5 rounded ${i < currentPhaseIndex ? 'bg-amber-500' : 'bg-gray-700'}`} />
                    )}
                  </React.Fragment>
                ))}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-4 bg-gray-900">
              {renderStep()}
            </div>
          </div>
        </div>
