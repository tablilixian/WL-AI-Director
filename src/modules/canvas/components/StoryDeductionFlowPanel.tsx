...
const DonePhase: React.FC<{ flow: FlowState; onClose: () => void; onReset: () => void }> = ({ flow, onClose, onReset }) => {
  ...
  return (
    <div className="py-12 text-center space-y-4">
      ...
      <h3 className="text-lg font-semibold text-[var(--text-primary)]">推演 + 视频生成完成！</h3>
      ...
      <div className="flex gap-2 justify-center">
        <button onClick={onClose} className="px-6 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700">
          关闭
        </button>
        <button onClick={onReset} className="px-6 py-2 border border-[var(--border-primary)] text-[var(--text-secondary)] text-sm rounded-lg hover:text-[var(--text-primary)] flex items-center gap-1.5">
...
export const StoryDeductionFlowPanel: React.FC<StoryDeductionFlowPanelProps> = ({ flowLayerId, onClose }) => {
  ...
  const handleClearFlow = useCallback(() => {
    ...
    const resetFlow: FlowState = { ...INITIAL_FLOW_STATE, sourceLayerId: flow.sourceLayerId, phase: 'select' };
    setFlow(resetFlow);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[300] flex items-center justify-center">
      ...
      <div
        ref={panelRef}
        className="relative bg-[var(--bg-layer)] rounded-xl border border-[var(--border-primary)] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"
      >
        {/* header */}
        <div className="flex-shrink-0 bg-[var(--bg-layer)] border-b border-[var(--border-primary)] px-5 py-3 flex items-center justify-between">
          <h2 className="text-sm font-bold text-[var(--text-primary)]">🎬 推演 → 视频</h2>
          <div className="flex items-center gap-1">
            {flow.phase !== 'select' && (
              <button onClick={handleClearFlow}
                className="text-[10px] text-[var(--text-tertiary)] hover:text-red-400 px-2 py-0.5 rounded hover:bg-red-500/10 transition-colors">
                重新开始
              </button>
            )}
            <button onClick={handleClose} className="p-1 hover:bg-gray-700 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)]">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {showSidebar && (
            <div className="flex-shrink-0 w-56 border-r border-[var(--border-primary)] bg-[var(--bg-base)] p-3 flex flex-col gap-3 overflow-y-auto">
              <div className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider flex items-center gap-1.5">
                <Image className="w-3 h-3" /> 源图片
              </div>
              <div className="rounded-lg overflow-hidden border border-[var(--border-primary)] bg-[var(--bg-hover)]">
                {sourceLayer?.src && (
                  <img src={sourceLayer.src} className="w-full object-cover" alt="源" />
                )}
              </div>
              <div className="text-[10px] text-[var(--text-muted)] truncate">{sourceLayer?.title || ''}</div>
              {sourceLayer && (
                  ...
                  {PHASE_ORDER.slice(0, 5).map((phase, i) => (
                    <div key={phase} className="flex items-center gap-2">
                      <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${
                        i < currentPhaseIndex ? 'bg-amber-500 text-white' :
                        i === currentPhaseIndex ? 'bg-amber-500/20 text-amber-400 border border-amber-500' :
                        'bg-[var(--bg-hover)] text-[var(--text-muted)] border border-[var(--border-primary)]'
                      }`}>
                        {i + 1}
                      </div>
                      <span className={`text-[10px] ${i <= currentPhaseIndex ? 'text-[var(--text-secondary)]' : 'text-[var(--text-tertiary)]'}`}>
                        {PHASE_LABELS[phase]}
                      </span>
                      {i < currentPhaseIndex && (
              ...
              <div className="flex items-center gap-1">
                {PHASE_ORDER.slice(0, 5).map((phase, i) => (
                  <React.Fragment key={phase}>
                    ...
                    {i < 4 && (
                      <div className={`flex-1 h-0.5 rounded ${i < currentPhaseIndex ? 'bg-amber-500' : 'bg-[var(--border-primary)]'}`} />
                    )}
                  </React.Fragment>
                ))}
              </div>
                ...
                {PHASE_ORDER.slice(0, 5).map((phase) => (
                  <span
                    ...
                    className={`text-[9px] font-medium ${
                      phase === flow.phase ? 'text-amber-400' : 'text-[var(--text-tertiary)]'
                    }`}
                  >
                    {PHASE_LABELS[phase]}
                  </span>
