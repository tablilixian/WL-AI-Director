...
export const PromptBar: React.FC<PromptBarProps> = () => {
  ...
  const { addLayer, updateLayer, suggestedPrompt, setSuggestedPrompt, setTemplatePanelOpen } =
    useCanvasStore();
        ...
        <div className="bg-gray-800/95 backdrop-blur-sm rounded-xl shadow-2xl border border-gray-700 p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            {mode === 'generate' && (
              <button
                ...
                className={`px-3 py-1 text-xs rounded-lg transition-colors ${
                  isAnime ? 'bg-pink-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                }`}
                title="使用动漫风格模型生成图像"
              >
                {isAnime ? '🎨 动漫' : '🎨 写实'}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={mode === 'generate' ? '描述你想生成的图片...' : '描述你想生成的视频...'}
