...
import { VlmAnalysisData, DEFAULT_ASPECTS } from '../../types/flow';
...
const DEFAULT_SYSTEM_PROMPT =
  '你是一个专业的影视镜头分析师。请从电影摄影的角度分析这张画面，输出结构化的分析结果。';

export const StepVLMAnalysis: React.FC<StepVLMAnalysisProps> = ({
  sourceLayerId,
  initialData,
  onSave,
  onNext,
  ...
  const [customUserPrompt, setCustomUserPrompt] = useState(initialData?.customUserPrompt || '');
  ...
  const [schema, setSchema] = useState<Record<string, string>>(initialData?.schema || {});
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {DEFAULT_ASPECTS.map((aspect) => (
                    <label
                      key={aspect.key}
                      className="flex items-center gap-1 px-2 py-0.5 rounded bg-[var(--bg-hover)] border border-[var(--border-primary)] cursor-pointer text-[10px] text-[var(--text-secondary)] hover:border-amber-500/50"
                    >
                      <input
                        type="checkbox"
                ...
                <OptimizableTextarea
                  ...
                  onOptimize={async () => {
                    const result = await optimizeVLMSystemPrompt({
                      ...
                      selectedAspectLabels: DEFAULT_ASPECTS.filter((a) =>
          ...
          <div className="bg-[var(--bg-base)] rounded-lg border border-amber-500/30 overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              {selectedAspects.map((key) => {
                ...
                return (
                  <div key={key}>
                    <label className="text-[10px] font-medium text-[var(--text-tertiary)] block mb-0.5">
                      {aspect.label}
                    </label>
                    <textarea
                      value={val}
