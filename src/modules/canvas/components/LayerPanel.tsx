...
export const LayerPanel: React.FC = () => {
  const { layers, selectedLayerIds, selectLayer, deleteLayer, searchLayers } = useCanvasStore();
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set());

  const filteredLayers = isSearching && searchQuery.trim() ? searchLayers(searchQuery) : layers;
      ...
      <div key={layer.id}>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm text-white truncate">{layer.title}</div>
            <div className="text-xs text-gray-500">
              {layer.type}
              {isGroup ? ` (${childLayers.length})` : ''}
            </div>
          </div>
        ...
        {isGroup && isExpanded && childLayers.length > 0 && (
          <div>{childLayers.map((child) => renderLayerItem(child, depth + 1))}</div>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="absolute top-24 right-4 z-50 w-64 bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-lg border border-gray-700">
      ...
      <div className="p-2 border-b border-gray-700">
        ...
        {isSearching && (
          <div className="text-xs text-gray-400 mt-1">找到 {filteredLayers.length} 个图层</div>
        )}
      </div>
