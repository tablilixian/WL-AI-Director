...
import { Scissors, Palette, ImagePlus, Grid3x3, ChevronDown, Brain, Orbit } from 'lucide-react';
  ...
  const groups: ActionGroup[] = [
    {
      ...
      single: { id: 'visual-language', label: 'AI 分析图片' },
    },
    {
      label: '基于此图生成',
            ...
            {group.single ? (
              ...
              <div className="relative">
                ...
                >
                  ...
                  {group.label}
                  <ChevronDown
                    className={`w-3 h-3 transition-transform ${openGroup === group.label ? 'rotate-180' : ''}`}
                  />
                </button>
      ...
      {activePanel &&
        createPortal(
          <>
            ...
            {activePanel === 'direct-style-transfer' && (
              <DirectStyleTransferPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'ipa-style-transfer' && (
              <IPAStyleTransferPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            ...
            {activePanel === 'multi-angle' && (
              <MultiAnglePanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'three-view' && (
              <ThreeViewPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'storyboard-deduction' && (
              <StoryboardDeductionPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'story-deduction-ai' && (
              <StoryDeductionPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'story-deduction' && (
              <DeductionPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'inpaint' && (
              <InpaintPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'lighting' && (
              <LightingControlPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'image-to-image' && (
              <ImageToImagePanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'visual-language' && (
              <VisualLanguagePanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
            {activePanel === 'panorama' && (
              <PanoramaPanel selectedLayerId={layer.id} onClose={handleClosePanel} />
            )}
          </>,
          document.body,
        )}
