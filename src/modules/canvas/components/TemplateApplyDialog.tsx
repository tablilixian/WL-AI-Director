...
export const TemplateApplyDialog: React.FC<Props> = ({ template, onClose }) => {
  ...
  const [useChinese, setUseChinese] = useState(() => (navigator.language || '').startsWith('zh'));
    ...
    try {
      ...
      if (resolvedUrl.startsWith('data:')) {
        try {
          ...
          logger.warn(LogCategory.CANVAS, '[TemplateApplyDialog] 保存图片失败:', e);
        }
      ...
      logger.error(LogCategory.CANVAS, '[TemplateApplyDialog] 生成失败:', error);
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="w-[560px] max-h-[85vh] bg-gray-800/95 backdrop-blur-sm rounded-xl shadow-2xl border border-gray-700 flex flex-col overflow-hidden">
          ...
          <div className="p-4 overflow-y-auto flex-1 space-y-4">
            <div
              className={`h-36 rounded-lg bg-gradient-to-br ${template.gradient} flex items-center justify-center relative overflow-hidden`}
            >
              {previewUrl ? (
                <button
            ...
            <div>
              ...
              <textarea
                ...
                disabled={isGenerating}
                placeholder={
                  useChinese ? '描述你想画的主体内容...' : 'Describe what you want to create...'
                }
              />
