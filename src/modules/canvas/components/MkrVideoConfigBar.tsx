...
import { X, Sparkles, Plus, Trash2, ChevronUp, ChevronDown, Film } from 'lucide-react';
...
export const MkrVideoConfigBar: React.FC<MkrVideoConfigBarProps> = ({ layerId }) => {
  ...
  const canGenerate = config.frames.length > 0 && !isGenerating;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const videoUrl = await canvasModelService.generateVideoMkr({
        prompt: fullPrompt,
        images,
        width: config.width,
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-700/60">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] text-gray-500 bg-gray-700/60 px-2 py-0.5 rounded-full">
              {config.frames.length} 关键帧 · {config.duration}s · {totalFrames}帧
            </span>
          </div>
