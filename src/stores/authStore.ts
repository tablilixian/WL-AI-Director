...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  initialize: async () => {
    console.log('[Auth] initialize:', { isValid: pb.authStore.isValid, model: pb.authStore.model?.id, token: pb.authStore.token?.slice(0, 20) })
  ...
  signIn: async (email, password) => {
    ...
    try {
      ...
      console.log('[Auth] signIn authData:', { recordId: authData.record?.id, token: authData.token?.slice(0, 20) + '...', modelId: pb.authStore.model?.id })
      ...
      setTimeout(() => {
        import('../../services/hybridStorageService').then(({ syncFromCloud, hybridStorage }) => {
          syncFromCloud().then((result: any) => {
            console.log(`[Auth] 登录同步完成: 上传 ${result.uploaded}, 下载 ${result.downloaded}`)
  ...
  signUp: async (email, password) => {
    ...
    try {
      ...
      // 注册后自动登录
      const authData = await pb.collection('users').authWithPassword(email, password)
      console.log('[Auth] signUp authData:', { recordId: authData.record?.id, token: authData.token?.slice(0, 20) + '...', modelId: pb.authStore.model?.id })
      set({ user: authData.record, loading: false })
      startTokenRefresh()
      ...
      setTimeout(() => {
        import('../../services/hybridStorageService').then(({ syncFromCloud, hybridStorage }) => {
          syncFromCloud().then((result: any) => {
            console.log(`[Auth] 注册同步完成: 上传 ${result.uploaded}, 下载 ${result.downloaded}`)
            if (result.uploaded > 0 || result.downloaded > 0) {
              window.dispatchEvent(new CustomEvent('projects-synced'))
            }
          }).catch(console.error)
