  ...
  async mergeVideos(
    ...
    for (let i = 0; i < completedShots.length; i++) {
      ...
      try {
        logger.debug(
          LogCategory.VIDEO,
          `[VideoMergeService] 正在下载视频 ${i + 1}/${completedShots.length}: ${videoUrl}`,
        );
        ...
        throw new Error(
          `下载视频片段 ${i + 1} 失败: ${err instanceof Error ? err.message : '未知错误'}`,
        );
      }
    }
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      ...
      mediaRecorder.onerror = (_event) => reject(new Error('录制失败'));
    });
