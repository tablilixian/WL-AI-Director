...
async function getDB(): Promise<IDBDatabase> {
  ...
  logger.info(LogCategory.STORAGE, '[CanvasStorage] IndexedDB 连接成功，版本:', dbInstance.version);
  return dbInstance;
}
...
export async function saveCanvasDataToLocal(
  ...
  logger.info(
    LogCategory.STORAGE,
    '[CanvasStorage] saveCanvasDataToLocal 被调用，projectId:',
    projectId,
  );

  try {
    const db = await getDB();
    logger.info(LogCategory.STORAGE, '[CanvasStorage] IndexedDB 连接成功');
    ...
    logger.info(LogCategory.STORAGE, '[CanvasStorage] 新版本号:', newVersion);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        logger.debug(
          LogCategory.CANVAS,
          `[CanvasStorage] 画布数据已保存到本地，项目: ${projectId}, 版本: ${newVersion}`,
        );
        logger.info(LogCategory.STORAGE, '[CanvasStorage] 保存成功，版本:', newVersion);
        resolve(canvasData);
      };

      request.onerror = () => {
        logger.error(LogCategory.CANVAS, `[CanvasStorage] 保存画布数据失败: ${request.error}`);
        logger.error(LogCategory.STORAGE, '[CanvasStorage] 保存失败:', request.error);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        if (request.result) {
          logger.debug(
            LogCategory.CANVAS,
            `[CanvasStorage] 从本地加载画布数据，项目: ${projectId}, 版本: ${request.result.version}`,
          );
        }
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        logger.debug(
          LogCategory.CANVAS,
          `[CanvasStorage] 获取到 ${request.result.length} 条待同步画布数据`,
        );
      ...
      request.onerror = () => {
        logger.error(
          LogCategory.CANVAS,
          `[CanvasStorage] 获取待同步画布数据失败: ${request.error}`,
        );
        reject(request.error);
      };
    });
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        logger.debug(
          LogCategory.CANVAS,
          `[CanvasStorage] 云端画布数据已保存到本地，项目: ${cloudData.projectId}`,
        );
        resolve();
      };
