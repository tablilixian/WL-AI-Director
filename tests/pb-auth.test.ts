...
describe('Auth Store - Sign In', () => {
  it('should authenticate user and set user state', async () => {
    ...
    mockCollection.mockImplementation((name: string) => {
      if (name === 'users') {
        return { authWithPassword: vi.fn().mockResolvedValue({ record: mockRecord, token: 'mock-token' }) }
      }
      return {}
    })
...
describe('Auth Store - Update Profile', () => {
  it('should update name and api_key', async () => {
    mockAuthStore.model = { id: 'user-1' }
    const mockUpdate = vi.fn().mockResolvedValue({ id: 'user-1', name: 'New Name', api_key: 'sk-xxx' })
    mockCollection.mockImplementation((name: string) => {
      if (name === 'users') {
        return { update: mockUpdate }
      }
