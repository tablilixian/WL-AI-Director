...
describe('HybridStorage - 云端同步修复 自测套件', () => {
  ...
  describe('Bug 5 修复: saveAssetToLibrary 返回更新后的 AssetLibraryItem', () => {
    it(
      '应该返回 AssetLibraryItem 类型 (在线模式)',
      record('返回类型-在线', async () => {
    ...
    it(
      '应该返回 AssetLibraryItem 类型 (离线模式)',
      record('返回类型-离线', async () => {
        // 模拟离线
  ...
  describe('Bug 2+3 修复: PB URL 回写到 data 中', () => {
    it(
      '角色只有 imageUrl 时，PB URL 应回写到 data.imageUrl',
      record('单图-角色-imageUrl', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-single',
        ...
        expect(result.data.imageUrl).toBe(
          'http://127.0.0.1:8090/api/files/coll-single/pb-single/asset_ts_main.jpg',
        );
      }),
    );

    it(
      '角色含 variations、threeView 时，所有 PB URL 应回写',
      record('多图-角色-全部字段', async () => {
        ...
        expect((result.data as any).threeViewImageUrl).toBe(`${baseUrl}/three_view.png`);
        expect((result.data as any).variations[0].imageUrl).toBe(`${baseUrl}/var_0.png`);
        expect((result.data as any).variations[1].imageUrl).toBe(`${baseUrl}/var_1.png`);
      }),
    );

    it(
      '角色含 turnaround、signaturePose、microAction 时，所有 PB URL 应回写',
      record('多图-角色-嵌套对象', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-nested',
        ...
        expect((result.data as any).turnaround.imageUrl).toBe(`${baseUrl}/turnaround.jpg`);
        expect((result.data as any).signaturePose.previewImageUrl).toBe(
          `${baseUrl}/sig_preview.webp`,
        );
        expect((result.data as any).microAction.previewImageUrl).toBe(
          `${baseUrl}/micro_preview.gif`,
        );
      }),
    );
  });
  ...
  describe('Bug 4 修复: 本地资产库记录同步更新 PB URL', () => {
    it(
      'saveAssetToLibrary 后，本地库中记录也应包含 PB URL',
      record('本地记录同步更新', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-local-update',
        ...
        expect(saved.data.imageUrl).toBe(
          'http://127.0.0.1:8090/api/files/coll-local/pb-local-update/main.png',
        );
      }),
    );
  });
  ...
  describe('Bug 1 修复: 不再调用 imageStorageService.uploadToCloud', () => {
    it(
      'saveAssetToLibrary 不应调用 imageStorageService.uploadToCloud',
      record('不再调用虚假uploadToCloud', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-no-fake',
  ...
  describe('场景和道具同步', () => {
    it(
      '场景保存后应回写 PB URL 到 data.imageUrl',
      record('场景同步', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-scene',
        ...
        const result = await hs.saveAssetToLibrary(item);
        expect(result.data.imageUrl).toBe(
          'http://127.0.0.1:8090/api/files/coll-scene/pb-scene/scene_main.jpg',
        );
      }),
    );

    it(
      '道具保存后应回写 PB URL 到 data.imageUrl',
      record('道具同步', async () => {
        mockPBCreate.mockResolvedValue({
          id: 'pb-prop',
        ...
        const result = await hs.saveAssetToLibrary(item);
        expect(result.data.imageUrl).toBe(
          'http://127.0.0.1:8090/api/files/coll-prop/pb-prop/prop_main.jpg',
        );
      }),
    );
  });
  ...
  describe('离线模式不触发云端同步', () => {
    it(
      '离线时不应调用 PB create/update',
      record('离线不触发云端同步', async () => {
        mockAuthStore.isValid = false;
