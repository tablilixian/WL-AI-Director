...
const ShotWorkbench: React.FC<ShotWorkbenchProps> = ({
  ...
  const activeCharacters =
    scriptData?.characters.filter((c) => shot.characters.includes(c.id)) || [];
  const availableCharacters =
    scriptData?.characters.filter((c) => !shot.characters.includes(c.id)) || [];
  const activeProps = (scriptData?.props || []).filter((p) => (shot.props || []).includes(p.id));
  const availablePropsForShot = (scriptData?.props || []).filter(
    (p) => !(shot.props || []).includes(p.id),
  );
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1 mr-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5">
              <span
                className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-widest truncate"
              ...
              >
                {nineGrid?.status === 'generating_panels' ? (
                  ...
                  <>
                    ...
                    <span>查看/确认镜头描述</span>
                    <span className="ml-1 px-1.5 py-0.5 bg-[var(--warning-text)]/10 rounded text-[8px]">
                      待确认
                    </span>
                  </>
                ) : nineGrid?.status === 'completed' ? (
                  <>
                    <Grid3x3 className="w-3.5 h-3.5" />
                    <span>查看九宫格分镜</span>
                    <span className="ml-1 px-1.5 py-0.5 bg-[var(--success-text)]/10 rounded text-[8px]">
                      Advanced
                    </span>
                  </>
                ) : (
                  <>
                    <Grid3x3 className="w-3.5 h-3.5" />
                    <span>九宫格分镜预览</span>
                    <span className="ml-1 px-1.5 py-0.5 bg-[var(--accent)]/10 text-[var(--accent-text)] rounded text-[8px]">
                      Advanced
                    </span>
                  </>
                )}
