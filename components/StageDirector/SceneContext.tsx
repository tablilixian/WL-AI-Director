        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1.5 overflow-hidden justify-center">
          ...
          <div className="relative group min-w-0">
            {onSceneChange && scenes.length > 1 ? (
              ...
              <div
                className="text-[var(--text-primary)] text-sm font-bold truncate"
                title={scene?.location}
              >
                {scene?.location || '未知场景'}
              </div>
          ...
          <div className="space-y-1">
            ...
            {scene?.atmosphere && (
              <p
                className="text-xs text-[var(--text-tertiary)] line-clamp-2 break-all leading-relaxed"
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[var(--border-secondary)] overflow-hidden flex-shrink-0">
                      {characterImageUrls[char.id] ? (
                        <img
                          src={characterImageUrls[char.id] ?? undefined}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded bg-[var(--border-secondary)] overflow-hidden flex-shrink-0">
                      {propImageUrls[prop.id] ? (
                        <img
                          src={propImageUrls[prop.id] ?? undefined}
                          className="w-full h-full object-cover"
                          alt={prop.name}
                        />
                      ) : (
                    ...
                    <span className="text-[9px] text-[var(--text-muted)] font-mono bg-[var(--bg-hover)] px-1 py-0.5 rounded">
                      {prop.category}
                    </span>
                  </div>
